...
router.get("/edit/:tugas_id", async function (req, res, next) {
  ...
  try {
    let data_user = await siswa_model.getByID(id);
    let data_tugas = await tugas_model.getByTugasID(tugas_id);
    let data_pengumpulan = await pengumpulan_model.getByTugasID(tugas_id)
...
router.get("/:mapelId", async function (req, res) {
  ...
  try {
    ...
    let pengumpulan_list = await pengumpulan_model.getByUserAndMapel(
      user_id,
      mapel_id
    );
...
router.post("/create", upload.single("file_tugas"), async (req, res) => {
  ...
  try {
    ...
    console.log(deadline[0].tanggal_deadline);
    if (tanggal_pengumpulan > deadline[0].tanggal_deadline) {
      const data = {
      ...
      tanggal_pengumpulan == deadline[0].tanggal_deadline &&
      waktu_pengumpulan > deadline[0].waktu_deadline
    ) {
      const data = {
        file_tugas: file_tugas,
        status: "telat",
...
router.post("/edit", upload.single("file_tugas"), async (req, res) => {
  const user_id = req.session.userId;
  const pengumpulan_id = req.params.pengumpulanID;
  const file_tugas = req.file.filename;
  const { tugas_id, mapel_id } = req.body;
  if (!file_tugas) {
    req.flash("messageError", "File Tidak ada !!");
  ...
  try {
    ...
    const waktu_pengumpulan =
      ...
      minutes;
    const deadline = await tugas_model.getByTugasID(tugas_id);
    console.log(tanggal_pengumpulan + ">" + deadline[0].tanggal_deadline);
    if (tanggal_pengumpulan > deadline[0].tanggal_deadline) {
      await pengumpulan_model.update("telat", tugas_id, user_id);
      console.log("File uploaded:", req.file.filename);
      req.flash("success", "Anda Telat Mengumpulkan");
      console.log("masuk 1");
    } else if (
      ...
      await pengumpulan_model.update("sudah", tugas_id, user_id);
      console.log("File uploaded:", req.file.filename);
      req.flash("success", "Anda Berhasil Mengumpulkan");
    }
    res.redirect("/siswa/tugas");
